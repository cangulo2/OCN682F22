...
#Method 2
length(mpg) #to obtain row numbers

#Method 3
str(mpg) #will give a lot of extra information, but also ncol and nrow
```
...
# Write your code here
ggplot(data = mpg) + 
 geom_point(mapping = aes(x = displ, y = hwy), color = "blue") #whatever goes inside the parentheses would need to communicate a value from the data set. Conversely, whatever goes outside would be an aesthetic choice.
```
...
```{r}
# Write your code here
mpg$class_numeric<- as.factor(mpg$class)
```
...
```{r}
# Write your code here
head(subset(mpg, cty = 15, select = c(manufacturer, model)))
...
sizeMass$size_log10 <- log10(sizeMass$size)
sizeMass$mass_log10 <- log10(sizeMass$mass)
...
## Method 2
ggplot(sizeMass, aes(size, mass)) +
 scale_x_log10() + geom_point() + geom_smooth(method = "lm", color = "red", se = FALSE)
```
